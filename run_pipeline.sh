...
if [ -f "data/raw/dolly_15k.jsonl" ]; then
    echo -e "${YELLOW}⚠️  Raw data already exists at data/raw/dolly_15k.jsonl${NC}"
    read -p "Force re-download? (y/N): " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        uv run python -m src.ingest
    else
        echo -e "${GREEN}⏭️  Skipping ingestion.${NC}"
    fi
else
    uv run python -m src.ingest
fi
...
echo -e "${BLUE}-------------------------------------------------------------${NC}"
echo -e "${YELLOW}⚙️  Stage 3/3: Distributed ETL (PySpark)${NC}"
echo -e "${BLUE}-------------------------------------------------------------${NC}"
